...
namespace QuanLyThuVien.Services
{
    public class DocGiaService
    {
        public DataTable LayDanhSach()
        {
            return Db.Query(@"
                SELECT dg.MaDocGia, dg.Ho, dg.Ten, dg.NgaySinh, dg.Phai,
                       ...
                       t.MaThe, t.NgayCap, t.HanSuDung,
                       t.DaDongLePhi, t.TrangThai
                FROM DocGia dg
                OUTER APPLY
                (
                    SELECT TOP 1 *
        ...
        {
            return Db.Query(
                "SELECT MaDocGia, (Ho + ' ' + Ten) AS HoTen " +
                "FROM DocGia ORDER BY Ho, Ten");
        }
            ...
            bool daDongLePhi)
        {
            ...
            using (SqlTransaction tx = cn.BeginTransaction())
            {
                try
                {
                    ...
                    return KetQuaXuLy.Ok(
                        "Cấp thẻ thư viện thành công. Mã thẻ: " + maThe);
                }
                catch (Exception ex)
                {
                    try
